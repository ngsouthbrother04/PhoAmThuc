...
| --- | --- | --- | --- | --- | --- |
| UC1 - Access & Authorization | AC-UC1 | TC-1.1 | TC-1.2 -> TC-1.5, TC-15.1, TC-15.2 | Auth API + payment callback flow + token lifecycle (`/api/v1/auth/token-refresh`, `/api/v1/auth/logout`), secure token storage, sync bootstrap (`/api/v1/sync/manifest`, `/api/v1/sync/full`) | ISSUE-002, ISSUE-003B |
...
| UC8 - Offline Content Access | AC-UC8 | TC-8.1 | TC-8.2, TC-8.3, TC-9.x, TC-10.x, TC-15.4 | Offline-first sync contract (manifest -> incremental -> full fallback), atomic SQLite replace (`pois`, `tours`, `sync_metadata`), file cache (`expo-file-system`) | ISSUE-005, ISSUE-006, ISSUE-003B, ISSUE-013 |
...
5. Khi mở rộng API contract, phải bổ sung test suite API completeness trong docs/test_scenarios.md (TC-18.x) trước khi đóng ISSUE-013.
...
- Sync path: `/api/v1/sync/manifest` compare -> `/api/v1/sync/incremental` when possible -> fallback `/api/v1/sync/full` when required.
