...
export const apiClient = {
  ...
  auth: {
    ...
    initiatePayment: async (provider, amount, currency = 'VND') => {
      const response = await authFetch(`${API_BASE_URL}/api/v1/auth/payment/initiate`, {
        method: 'POST',
        body: JSON.stringify({
          provider,
          amount,
    ...
    redeemClaimCode: async (code) => {
      const response = await authFetch(`${API_BASE_URL}/api/v1/auth/payment/claim`, {
        method: 'POST',
  ...
  pois: {
    ...
    searchByRadius: async (latitude, longitude, radiusM, limit = 20) => {
      const response = await authFetch(`${API_BASE_URL}/api/v1/pois/search/radius`, {
        method: 'POST',
        body: JSON.stringify({
          latitude,
          longitude,
  ...
  tours: {
    ...
    getById: async (tourId) => {
      const response = await authFetch(`${API_BASE_URL}/api/v1/tours/${tourId}`);
      return response.json();
    },
  ...
  sync: {
    ...
    getIncremental: async (fromVersion) => {
      const response = await authFetch(`${API_BASE_URL}/api/v1/sync/incremental`, {
        method: 'POST',
  ...
  analytics: {
    submitEvents: async (events) => {
      const response = await authFetch(`${API_BASE_URL}/api/v1/analytics/events`, {
        method: 'POST',
    ...
    getStats: async () => {
      const response = await authFetch(`${API_BASE_URL}/api/v1/analytics/stats`);
      return response.json();
    }
  },
