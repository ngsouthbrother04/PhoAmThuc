...
export function verifyVNPaySignature(vnpayParams: Record<string, string>): boolean {
  ...
  const signData = keys
    .map((k) => `${k}=${encodeURIComponent(vnpayParams[k].toString()).replace(/%20/g, '+')}`)
    .join('&');

  const generatedHash = crypto.createHmac('sha512', secretKey).update(Buffer.from(signData, 'utf-8')).digest('hex');

  return secureHash.toLowerCase() === generatedHash.toLowerCase();
}

export function verifyMoMoSignature(momoParams: Record<string, string>): boolean {
